app.factory('esconsole', ['$window', function ($window) {
    ...
    var ESConsoleTraceLevel = 2;
    var ESConsoleExcludedTags = ['DEBUG'];
    ...
    function getURLParameters() {
        ...
        var query = wdw.location.hash.substring(2); // replaced with location.search considering # always being used in other URL queries
 ...
 * } @ caller function
    ...
    function esconsole(message, tag, traceLevel, logLevel) {
        var date = new Date();
        var log = date.toLocaleTimeString('en-GB') + '.' + ('000'+date.getMilliseconds()).slice(-3) + ' ';
        var output;
        var TAG;
        var trace;
